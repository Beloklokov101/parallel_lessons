...
typedef struct {
	...
	MPI_Datatype type_col, type_row, type_block;
    MPI_Comm decart_comm, row_comm, col_comm;
} life_t;
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < l.steps; i++) {
		if (i % l.save_steps == 0) {
			sprintf(buf, "outGather/life_%06d.vtk", i);
			life_gather3(&l);
...
void life_init(const char *path, life_t *l)
{
    ...
    MPI_Comm_split(l->decart_comm, l->coords[1], l->coords[0], &(l->row_comm));
    MPI_Comm_split(l->decart_comm, l->coords[0], l->coords[1], &(l->col_comm));
    // l->coords = &coords;
    ...
    MPI_Type_vector(l->end_row - l->start_row, 1, l->nx, MPI_INT, &(l->type_col));
	MPI_Type_commit(&(l->type_col));

    int row_length = l->end_col - l->start_col;
    if (l->coords[0] == 0){
        int count = 2;
    ...
    MPI_Type_vector(l->end_row - l->start_row, l->nx / l->dims[0], l->nx, MPI_INT, &(l->type_block));
    MPI_Aint lb = 0, extent = (l->end_col - l->start_col) * sizeof(int);
    MPI_Type_create_resized(l->type_block, lb, extent, &(l->type_block));
...
void life_free(life_t *l)
{
    ...
    MPI_Comm_free(&(l->row_comm));
    MPI_Comm_free(&(l->col_comm));
...
void life_exchange(life_t *l)
{
    ...
    int coords_right[2] = {l->coords[0] + 1, l->coords[1]};
    int coords_left[2] = {l->coords[0] - 1, l->coords[1]};
    int coords_up[2] = {l->coords[0], l->coords[1] - 1};
    int coords_down[2] = {l->coords[0], l->coords[1] + 1};
...
void life_gather3(life_t *l)
{
    MPI_Gather(l->u0 + ind(l->start_col, l->start_row), 1, l->type_block,
               l->u0 + ind(0, l->start_row), 1, l->type_block, l->dims[0] - 1, l->row_comm);
    
    if (l->coords[0] == l->dims[0] - 1)
    {
        MPI_Gather(l->u0 + ind(0, l->start_row), l->nx * (l->ny / l->dims[1]), MPI_INT,
                   l->u0 + ind(0, 0), l->nx * (l->ny / l->dims[1]), MPI_INT, l->dims[1] - 1, l->col_comm);
    }
}