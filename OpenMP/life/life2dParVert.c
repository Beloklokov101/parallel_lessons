...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < l.steps; i++) {
		if (i % l.save_steps == 0) {
			sprintf(buf, "outVert/life_%06d.vtk", i);
...
{
	if (l->rank % 2 == 0){
		MPI_Send(l->u0 + ind(l->end - 1, 0), 1, l->type_col, (l->rank + 1) % l->size, 0, MPI_COMM_WORLD);
		...
		MPI_Recv(l->u0 + ind(l->end, 0), 1, l->type_col, (l->rank + 1) % l->size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	} else {
		MPI_Recv(l->u0 + ind(l->start - 1, 0), 1, l->type_col, (l->rank - 1 + l->size) % l->size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Send(l->u0 + ind(l->end - 1, 0), 1, l->type_col, (l->rank + 1) % l->size, 0, MPI_COMM_WORLD);
...
void life_gather2(life_t *l)
{
	if (l->rank == l->size - 1) {
		...
		l->type_block, l->size - 1, 0, MPI_COMM_WORLD);
	}
}
