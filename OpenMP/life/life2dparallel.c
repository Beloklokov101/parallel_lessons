...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < l.steps; i++) {
		if (i % l.save_steps == 0) {
			sprintf(buf, "life/out/life_%06d.vtk", i);
...
{
	if (l->rank % 2 == 0){
		MPI_Send(l->u0 + ind(0, l->end - 1), l->nx, MPI_INT, (l->rank + 1) % l->size, 0, MPI_COMM_WORLD);
		...
		MPI_Recv(l->u0 + ind(0, l->end), l->nx, MPI_INT, (l->rank + 1) % l->size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	} else {
		MPI_Recv(l->u0 + ind(0, l->start - 1), l->nx, MPI_INT, (l->rank - 1 + l->size) % l->size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Send(l->u0 + ind(0, l->end - 1), l->nx, MPI_INT, (l->rank + 1) % l->size, 0, MPI_COMM_WORLD);
...
void life_gather(life_t *l)
{
    if (l->rank == 0){
        ...
        for (i = 1; i < l->size; i++) {
            ...
            MPI_Recv(l->u0 + ind(0, start), (end - start) * l->nx, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    } else {
        MPI_Send(l->u0 + ind(0, l->start), (l->end - l->start) * l->nx, MPI_INT, 0, 0, MPI_COMM_WORLD);
    }
}